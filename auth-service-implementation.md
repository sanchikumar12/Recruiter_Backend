...
## 13b. Password Reset Token Migration
...
## 24. Registration (Server-Generated Credentials)
...
Request (Password is optional; if omitted, the server automatically generates a secure unique random password):
...
Or optionally with custom password:
...
```json
{
  "message": "User registered successfully. Temporary login credentials have been generated.",
  "email": "candidate@example.com",
  "generatedPassword": "K9#mX2$pQ7!v",
  "status": "ACTIVE"
}
```
...
Hash password with BCrypt
     |
Create AuthUser (Status: ACTIVE, Role: CANDIDATE)
     |
In dev: Return credentials in API response
In prod: Send credentials to candidate via Email Service
```
...
## 34. Forgot Password & Reset Password
...
```json
{
  ...
  "resetToken": "a2d4f8b9-1234-5678-9abc-def012345678"
}
```
...
```json
{
  "token": "a2d4f8b9-1234-5678-9abc-def012345678",
...
Generate random reset token (15-min expiry)
       |
Store token SHA-256 hash in password_reset_tokens
       |
In prod: Send email with reset token / link
       |
...
BCrypt hash new password & save
       |
Invalidate reset token (used = true)
       |
